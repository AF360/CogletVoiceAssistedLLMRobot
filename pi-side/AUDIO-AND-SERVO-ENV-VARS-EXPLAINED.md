...
| 1. Recorder/MIC |  |  |  |
| MIC_SR | "16000" | pi-side/coglet-local.py, Line 148 | Is passed as Sample-Rate to the recorder: rec = Recorder(sr=MIC_SR, vad_aggr=VAD_AGGR) (coglet-local.py, Line 2193). The recorder then works with exactly this rate; this rate is also passed on to SpeechEndpoint (rec.sr in lines 2340 and 2408). |
| MIC_DEVICE | "0" (in the Recorder) | coglet-local.py, Lines 1483–1484 | The recorder reads MIC_DEVICE directly: dev_env = os.getenv("MIC_DEVICE", "0"). If the value consists only of digits, it is interpreted as an index, otherwise as a device name. Thus controls which input device sounddevice.RawInputStream uses. |
| MIC_GAIN_DB | "0" | coglet-local.py, Line 1485 (Recorder) and Line 149 (global constant) | In the recorder, self.gain_db = float(os.getenv("MIC_GAIN_DB", "0")) is set, and a linear gain is calculated from it (self._lin_gain, Line 1491). Affects the amplification for read() → float32 (Excerpt Lines 1490–1492). |
| MIC_AUTO_GAIN | "0" | coglet-local.py, Line 1486 and Line 150 | In the recorder: self.auto_gain = os.getenv("MIC_AUTO_GAIN", "0") in ("1", "true", "True"). Serves as a flag for automatic gain control (AGC); the actual AGC logic would be in other methods (not directly relevant for start/end of recording), but is at least logged (Lines 1506–1513). |
| MIC_TARGET_DBFS | "-18" | coglet-local.py, Line 1487 and Line 151 | Target level for a possible AGC: self.target_dbfs = float(os.getenv("MIC_TARGET_DBFS", "-18")) (Line 1487). Currently only visible in logging (Lines 1506–1513). |
| MIC_MAX_GAIN_DB | "35" | coglet-local.py, Line 1488 and Line 152 | Maximum software gain: self.max_gain_db = float(os.getenv("MIC_MAX_GAIN_DB", "35")). Is also output in the start log (Lines 1506–1513). |
|  |  |  |  |
| 2. VAD / Endpointing (SpeechEndpoint) |  |  |  |
| VAD_AGGRESSIVENESS | "2" | coglet-local.py, Line 154 (as VAD_AGGR) | The value is passed to the recorder (Recorder(sr=MIC_SR, vad_aggr=VAD_AGGR), Line 2193), passed through from there to SpeechEndpoint(sr=rec.sr, vad_aggr=rec.vad_aggr) (Lines 2340 and 2408) and finally used for webrtcvad.Vad(int(vad_aggr)) (coglet-local.py, Lines 1388–1389). Thus controls the sensitivity of the WebRTC-VAD. |
| VAD_FRAME_MS | "30" | coglet-local.py, Line 1392 | Sets the frame length in milliseconds. The number of samples per frame is calculated as self.frame_samples = (self.sr * self.frame_ms) // 1000, the byte size as self.frame_bytes (Lines 1408–1410). record() reads exactly frame_bytes each time (recorder.read_bytes(self.frame_bytes), Line 1435). |
| VAD_START_WIN | "5" | coglet-local.py, Line 1393 | Window size for the majority decision: votes = collections.deque(maxlen=self.start_win) (Line 1419). For Start-VAD, 0/1 votes are pushed into this window per frame (Lines 1441–1445). |
| VAD_START_MIN | "3" | coglet-local.py, Line 1394 | Minimum number of Speech votes in the start window: if len(votes) == self.start_win and sum(votes) >= self.start_min ... (Lines 1447–1448). |
| VAD_START_CONSEC_MIN | "3" | coglet-local.py, Line 1395 | Minimum number of consecutive speech frames: consec_speech = (consec_speech + 1) if is_speech else 0 (Line 1445) and condition ... and consec_speech >= self.start_consec (Line 1447). |
| VAD_END_HANG_MS | "400" | coglet-local.py, Line 1396 | Length of the "hangover" phase after silence in milliseconds. From this, self.hang_frames = max(1, math.ceil(self.end_hang_ms / self.frame_ms)) is formed (Line 1411). In End-VAD, the process is aborted when frames_since_end >= self.hang_frames (Lines 1456–1463). |
| VAD_END_GUARD_MS | "1200" | coglet-local.py, Line 1397 | Minimum duration from speech start until allowed ending: self.end_guard_s = self.end_guard_ms / 1000.0 (Line 1413). In record(), this limit is compared with local_end_guard: if frames_since_end >= self.hang_frames and (now - started_at) >= local_end_guard: (Lines 1460–1463). |
| VAD_PREROLL_MS | "240" | coglet-local.py, Line 1398 | Length of the preroll phase: self.preroll_frames = max(0, self.preroll_ms // self.frame_ms) (Line 1412). Before start, frames are saved in preroll (Lines 1443–1444); when start is detected, these frames are also written to the buffer (Lines 1448–1450). |
| MAX_UTTER_S | "8.0" | coglet-local.py, Line 1399 | Hard upper limit per record() call. In the loop: if (now - start_ts) > (self.max_utter if speech_started else local_no_speech): break (Line 1432). Applies from speech start (when speech_started == True). |
| NO_SPEECH_TIMEOUT_S | "3.0" | coglet-local.py, Line 1400 | Timeout before detected speech start. Becomes local_no_speech in record() if no argument is passed (Line 1416). As long as speech_started == False is true: if (now - start_ts) > local_no_speech: break (Line 1432). |
|  |  |  |  |
| 3. Follow-up Mode & TTS Gating |  |  |  |
| FOLLOWUP_ENABLE | "1" | coglet-local.py, Line 2389 | If os.getenv("FOLLOWUP_ENABLE", "1") is in ("1", "true", "True"), the follow-up block is activated after every response. For other values, the entire follow-up mode is skipped. |
| FOLLOWUP_MAX_TURNS | "10" | coglet-local.py, Line 2391 | Is read into max_turns = int(os.getenv("FOLLOWUP_MAX_TURNS", "10")). In the loop condition: while ... and (max_turns == 0 or turns < max_turns) (Lines 2397–2400). 0 means an unlimited number of follow-up turns, otherwise a hard upper limit. |
| FOLLOWUP_ARM_S | "3.0" | coglet-local.py, Line 2394 | Is set as arm_s and then passed directly to record(): endpoint.record(rec, no_speech_timeout_s=arm_s) (Line 2411). Thus, arm_s in the follow-up overwrites the standard NO_SPEECH_TIMEOUT_S: if no speech start is detected within this time window, record() aborts (Lines 1416 and 1432). |
| FOLLOWUP_COOLDOWN_S | "0.10" | coglet-local.py, Line 2395 | Short waiting time before each follow-up listen: time.sleep(fu_cd) (Line 2403), followed by rec.flush() (Line 2404). Serves to remove TTS echo from the buffer before listening again. |
| BARGE_IN | Default True | coglet-local.py, Line 135; Evaluation in Lines 1252, 1880–1889 | Is set via _parse_bool(os.getenv("BARGE_IN"), True). If True, the microphone remains active during TTS (half_duplex_tts(), Lines 1248–1255). If False, _listen is deactivated during TTS and reactivated after TTS with additional cooldown and buffer flush (speak_and_back_to_idle, Lines 1880–1895). Thus influences whether audio enters the pipelines during speech output. |
| COOLDOWN_AFTER_TTS_S | "0.5" | coglet-local.py, Line 1890 | Only effective if BARGE_IN is False. After TTS, post_cd = float(os.getenv("COOLDOWN_AFTER_TTS_S", "0.5")) is set and – if > 0 – waited via time.sleep(post_cd) (Lines 1890–1892). Subsequently, input buffers are cleared (_flush_input_buffers(recorder)) and the wakeword is newly "armed" (activated) (kw.reset_after_tts(), Lines 1893–1894). |
|  |  |  |  |
| Servo-related variables: |  |  |  |
| 1. Face tracking master switch & Grove Vision AI UART |  |  |  |
| FACE_TRACKING_ENABLED | "1" | coglet-local.py, lines 195, 539–540, 2106, 2210 | Global on/off switch for face tracking. If set to 0, _create_face_tracker() returns None and tracking is disabled. |
| FACE_TRACKING_SERIAL_PORT | "/dev/ttyACM0" | coglet-local.py, line 554 | UART device used by GroveVisionAIClient. |
| FACE_TRACKING_BAUDRATE | "921600" | coglet-local.py, line 555 | Baud rate passed to GroveVisionAIClient constructor. |
| FACE_TRACKING_SERIAL_TIMEOUT | "0.0" | coglet-local.py, line 556 | Read timeout (seconds) for the Grove Vision AI client. |
|  |  |  |  |
| 2. PCA9685 frequency & servo channel selection |  |  |  |
| FACE_TRACKING_PWM_FREQ_HZ | "50.0" | coglet-local.py, line 349 | PWM frequency (Hz) applied to all servos used for face tracking. |
| FACE_TRACKING_EYE_CHANNELS | "0,1" | coglet-local.py, lines 351–359 | Base list of PCA9685 channels for eyes (EYL, EYR). Parsed via resolve_channel_list. |
| FACE_TRACKING_WHEEL_CHANNELS | "8,9" | coglet-local.py, lines 361–363, 386–395 | Base list of PCA9685 channels for wheels (LWH, RWH). Empty list disables wheel tracking. |
| FACE_TRACKING_EYE_LEFT_CHANNEL | "0" | coglet-local.py, lines 397–399 | Overrides channel for EYL (left eye). |
| FACE_TRACKING_EYE_RIGHT_CHANNEL | "1" | coglet-local.py, lines 400–402 | Overrides channel for EYR (right eye). |
| FACE_TRACKING_YAW_CHANNEL | "" (empty) | coglet-local.py, lines 369–411 | Optional channel(s) for NRL (yaw). Empty means yaw servo is disabled for tracking. |
| FACE_TRACKING_PITCH_CHANNEL | "3" | coglet-local.py, lines 369–417 | Optional channel(s) for NPT (pitch). Empty means pitch servo is disabled for tracking. |
| FACE_TRACKING_WHEEL_LEFT_CHANNEL | "8" | coglet-local.py, lines 419–421 | Overrides channel for LWH (left wheel). |
| FACE_TRACKING_WHEEL_RIGHT_CHANNEL | "9" | coglet-local.py, lines 422–423 | Overrides channel for RWH (right wheel). |
|  |  |  |  |
...
| P_MIN_ANGLE_DEG |  | coglet-local.py, lines 231–244 | If a specific env var is missing or invalid, the corresponding value from the base layout (SERVO_LAYOUT_V1) is used instead |
| P_MAX_ANGLE_DEG |  |  |  |
| P_MIN_PULSE_US |  |  |  |
| P_MAX_PULSE_US |  |  |  |
...
| EYELID_OPEN_DEG | layout_cfg.neutral_deg (LID’s neutral angle) | coglet-local.py, lines 298–300 | Absolute angle for “eyes open”. Clamped between servo min/max. |
| EYELID_CLOSED_DEG | open_angle - 60.0 (clamped between servo min/max) | coglet-local.py, lines 300–303 | Absolute angle for “eyes closed”. |
| EYELID_SLEEP_FRACTION | 0.7 (clamped to 0.0–1.0) | coglet-local.py, lines 304–305 | Fraction between open/closed used for “sleepy” half-closed state. |
| EYELID_BLINK_MIN_S | 3.0 | coglet-local.py, line 306 | Minimum random interval between blinks. |
| EYELID_BLINK_MAX_S | 7.0 | coglet-local.py, line 307 | Maximum random interval between blinks. |
| EYELID_BLINK_CLOSE_S | 0.06 | coglet-local.py, line 308 | Duration for the closing phase of a blink. |
| EYELID_BLINK_HOLD_S | 0.04 | coglet-local.py, line 309 | Time to hold eyes closed. |
| EYELID_BLINK_OPEN_S | 0.07 | coglet-local.py, line 310 | Duration for the opening phase of a blink. |
|  |  |  |  |
| 7. Generic animation servo overrides |  |  |  |
| For non-tracking servos (e.g. mouth, ears, head roll, etc.), _create_servo_setup() uses prefix = f"ANIM_{name}" for any servo name not explicitly listed in prefix_map (coglet-local.py, lines 472–490). |  |  |  |
...
| ANIM_EAR_INVERT |  |  |  |
| All these names are resolved purely via _create_servo_config() (coglet-local.py, lines 217–244) and fall back to the corresponding SERVO_LAYOUT_V1 defaults in servo_presets.py if not set. |  |  |  |
