...
#### USB firmware limitations on the ReSpeaker Lite

Unlike the older ReSpeaker USB/Mic Array v2/v3 boards, the Lite with USB firmware has **no** vendor endpoint for LED commands. The single WS2812 RGB LED is wired directly to XMOS GPIO 1 and is only driven by firmware demos on the device (e.g., the I²S firmware via I²C-CTP, RESID 0xF1). On the host side every `ctrl_transfer(..., wIndex=0x1C, …)` call is ignored and the USB stack returns `PIPE/STALL`. To control the LED on the ReSpeaker Lite you would need to use the I²S firmware and change the code from USB to I²S.
