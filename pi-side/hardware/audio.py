...
class Recorder:
    ...
    def __init__(self, sr: int = 16000, vad_aggr: int = 2):
        ...
        self.channels = max(1, int(os.getenv("MIC_CHANNELS", "1")))
        self.channel_index = int(os.getenv("MIC_CHANNEL_INDEX", "0"))
        ...
        logger.info(
            "[audio] MIC setup: device=%s sr=%s channels=%s channel_index=%s gain=%.1fdB agc=%s",
            self.device, self.sr, self.channels, self.channel_index, self.gain_db, self.auto_gain
        )
    ...
    def _callback(self, indata, frames, time_info, status):
        ...
        if self.channels > 1:
            channel_index = min(max(0, self.channel_index), self.channels - 1)
            data = np.frombuffer(indata, dtype="<i2")
            if data.size < self.channels:
                return
            frame_count = data.size // self.channels
            data = data[:frame_count * self.channels].reshape(frame_count, self.channels)
            data_mono = data[:, channel_index]
    ...
    def trim_buffer(self, max_seconds: float) -> None:
        max_bytes = max(0, int(float(max_seconds) * self.sr) * 2)
        if max_bytes <= 0:
            self.flush()
            return
...
class Wakeword:
    ...
    def __init__(self, backend: str, model_path: str, threshold: float, hw_sr: int):
        ...
        M = 1280
        self.predict_mode = os.getenv("OWW_PREDICT_MODE", "stream").strip().lower()
        if self.predict_mode not in {"stream", "window"}:
            self.predict_mode = "stream"
        win_ms = int(os.getenv("OWW_WIN_MS", "800"))
        hop_default = "80" if self.predict_mode == "stream" else "160"
        hop_ms = int(os.getenv("OWW_HOP_MS", hop_default))
        ...
        self.debug = int(os.getenv("OWW_DEBUG", "0"))
        self.score_log_interval_s = float(os.getenv("OWW_SCORE_LOG_INTERVAL_S", "1.0"))
        self._last_score_log_ts = 0.0
        ...
        self._stream_buf = np.empty(0, dtype=np.float32)

        logger.info("[oww] Loading model %s (thr=%.2f)", os.path.basename(model_path), threshold)
        if self.debug:
            logger.info(
                "[oww] Audio route: hw_sr=%s oww_sr=%s mode=%s win_ms=%s hop_ms=%s hop_hw=%s",
                self.hw_sr, self.oww_sr, self.predict_mode, win_ms, hop_ms, self.hop_hw
            )
        self.detector = _OWWModel(wakeword_models=[model_path], inference_framework="onnx")


        probe_samples = M if self.predict_mode == "stream" else self.win_oww
        probe = np.zeros(probe_samples, dtype=np.int16)
    ...
    def check_once(self, recorder: Recorder) -> bool:
        ...
        score = self._score_audio(y)


        now = time.monotonic()
        self._log_score(score, now)
    ...
    def _score_audio(self, y: np.ndarray) -> float:
        if self.predict_mode == "window":
            self.ring = np.roll(self.ring, -y.size)
            self.ring[-y.size:] = y
            return self._predict(self.ring)

        self._stream_buf = np.concatenate((self._stream_buf, y))
        score = 0.0
        frame_size = 1280
        while self._stream_buf.size >= frame_size:
            frame = self._stream_buf[:frame_size]
            self._stream_buf = self._stream_buf[frame_size:]
            score = max(score, self._predict(frame))
        return score
    ...
    def _log_score(self, score: float, now: float) -> None:
        if not self.debug:
            return
        near_hit = score >= (self.threshold * 0.5)
        due = now - self._last_score_log_ts >= self.score_log_interval_s
        if self.debug < 2 and not (near_hit or due):
            return
        self._last_score_log_ts = now
        logger.info(
            "[oww] score=%.3f threshold=%.3f armed=%s key=%s",
            score, self.threshold, self.armed, self.key
        )
