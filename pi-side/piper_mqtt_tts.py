...
PIPER_MODEL= os.getenv("PIPER_MODEL", "/opt/piper/voices/de_DE-thorsten-high.onnx")
PIPER_CFG  = os.getenv("PIPER_CFG",   "/opt/piper/voices/de_DE-thorsten-high.onnx.json")
...
def _remove_pending(eid: str) -> bool:
    ...
    removed = False
    with say_q.mutex:
        items = list(say_q.queue)
        say_q.queue.clear()
        for item in items:
            if item[0] == eid:
                removed = True
                continue
            say_q.queue.append(item)
    return removed
...
def worker_loop(client: mqtt.Client, piper: PiperPersistent, player: Player, cancel_flag):
    ...
    while True:
        ...
        try:
            ...
            publish_status(client, "SPEAKING", eid)
