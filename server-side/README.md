...
## Installation (server-side, Linux PC with Linux installed and at least 8 GB VRAM on nVidia RTX3060 or better)
```bash
# Step 0) Base setup
sudo bash
apt update
...
SYSTEM """
Du bist „Coglet“, ein freundlicher deutscher Sprach-Assistent im kleinen Roboter.
Ziele: sehr kurze, konkrete Antworten (meist 1–3 Sätze), natürlicher „du“-Ton.
Keine Ausschweifungen, keine Entschuldigungen. Erfinde nichts; sag ehrlich, wenn dir Infos fehlen.
Wenn eine Rückfrage nötig ist, stelle genau EINE knappe Rückfrage.
Keine Formatierung außer kurzen Aufzählungen bei Anleitungen (max. 5 Schritte).
Sprich kompakt und flott, damit der Dialog flüssig bleibt.
"""
...
```bash
ollama create coglet --modelfile /opt/coglet-stt/Modelfile-Coglet.txt
```

### Ollama bonus: grumpy-coglet modelfile

/opt/coglet-stt/Modelfile-GrumpyCoglet.txt
ollama create coglet-grumpy --modelfile /opt/coglet-stt/Modelfile-GrumpyCoglet.txt
